...
| B | 5-fold GroupKFold on `subject_id` | Main comparison table. Configs: `configs/protocol_b_*.yaml`, `watch_xgb.yaml`, `fusion_xgb.yaml`. |
| C | Grouped holdout: 5 test subjects, rest train, 3 repeats from one seed (46/5 x 3). Not 51-fold LOSO. | Variance across people. Config: `configs/protocol_c_loso.yaml` (filename kept; split is `grouped_holdout`). |
...
Target after A2 exists: leaky accuracy in the same ballpark as 0.86 is possible but not required. Protocol B on the **same** flattened features should drop. That drop is the finding (RQ2). Config: `configs/protocol_b_raw_flat.yaml` (student XGBoost params, GroupKFold).

**Fusion** in this repo is concat of phone and watch windows. Each window is still 6 channels. `data.device: both` loads both devices. It is not a 12-channel time-aligned phone+watch fusion. Config: `configs/fusion_xgb.yaml`.
...
Honest B/C XGBoost (phone statistical, watch, concat fusion, Protocol C) uses a smaller family: `n_estimators: 200`, `max_depth: 6`, `device: cuda`. Do not treat those runs as a 982-tree reproduction.
...
python -m har.train --config configs/protocol_b_logreg.yaml
python -m har.train --config configs/protocol_b_rf.yaml
python -m har.train --config configs/protocol_b_groupkfold.yaml
python -m har.train --config configs/protocol_b_raw_flat.yaml
python -m har.train --config configs/watch_xgb.yaml
python -m har.train --config configs/fusion_xgb.yaml
python -m har.train --config configs/protocol_c_loso.yaml

python -m har.evaluate --configs configs/protocol_b_dummy.yaml configs/protocol_b_logreg.yaml
