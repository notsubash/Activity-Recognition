...
def write_data_card(
    ...
    *,
    ...
    lines = [
        "# WISDM data card",
        "",
        "Audit of the raw [WISDM Smartphone and Smartwatch Activity and Biometrics "
        "Dataset (UCI 507)](https://archive.ics.uci.edu/dataset/507/"
        "wisdm+smartphone+and+smartwatch+activity+and+biometrics+dataset) dump on disk. "
        "This card describes raw sessions, not the repaired 20 Hz parquet. "
        "Regenerate with `python -m har.data.audit` (or `python scripts/audit.py`).",
        "",
        "Sampling rate is implied Hz: `1e9 / median_dt_ns` from timestamp diffs "
        "inside a session. Absolute Unix epoch is unused.",
        "",
        "## Row counts vs Weiss",
        "",
    ...
    lines.extend(
        [
            ...
            "- Start-of-trial seconds may not match the labeled activity. "
            "Repair can drop them (`trim_start_s`); the default tree is untrimmed.",
            "- Accel and gyro clocks do not share sample instants. "
            "Repair interpolates both onto a shared 20 Hz grid per session "
